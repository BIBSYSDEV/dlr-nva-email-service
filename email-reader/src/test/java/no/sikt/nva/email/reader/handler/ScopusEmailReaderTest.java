...
public class ScopusEmailReaderTest {
  ...
  private static final String FULL_ABSTRACTS =
      "s3://some-bucket/2023-6-14_ANI-ITEM-full-format-xml.zip";
  ...
  public static Stream<Arguments> generateInvalidSenderEmail() throws MimeException, IOException {
    return Stream.of(
        Arguments.of(EmailGenerator.generateEmailWithInvalidSubject()),
  ...
  void shouldThrowExceptionIfItCannotParseTheEmail() throws IOException {
    ...
    assertThat(
        exception.getObjectKey(), is(equalTo(extractObjectKey(s3EventNotContainingAnEmail))));
  }

  @ParameterizedTest(
      name = "should throw exception when email sender is not from Elsevier email account")
  @MethodSource("generateInvalidSenderEmail")
  void shouldThrowExceptionWhenEmailSenderIsNotFromElsevierEmailAccount(String invalidEmails)
      throws IOException {
    var s3EventNotFromSikt = createS3Event(invalidEmails);
    var exception =
        assertThrows(
            EmailException.class, () -> handler.handleRequest(s3EventNotFromSikt, CONTEXT));
    assertThat(exception.getMessage(), containsString(COULD_NOT_VERIFY_EMAIL));
  ...
  @Test
  void shouldThrowExceptionIfEmailDoesNotHaveContentTypeMultipart()
      throws IOException, MimeException {
    var s3Event = createS3Event(EmailGenerator.generateNonMultipartEmail());
    var exception =
        assertThrows(EmailException.class, () -> handler.handleRequest(s3Event, CONTEXT));
  ...
  void shouldThrowExceptionIfTheURIsInTheEmailIsNotDownloadable() throws IOException {
    ...
    handler =
        new ScopusEmailReader(
            s3Client, new FakeZipFileRetrieverThrowingException(), SCOPUS_ZIP_BUCKET);
    var exception =
        assertThrows(
            EmailException.class, () -> handler.handleRequest(s3EventNotFromSikt, CONTEXT));
  ...
  private Set<URI> urlsInValidEmailTxt() {
    return Set.of(
        UriWrapper.fromUri(FULL_ABSTRACTS).getUri(), UriWrapper.fromUri(DELETE_LIST).getUri());
  }
  ...
  private S3EventNotification.S3Entity createS3Entity(String expectedObjectKey) {
    var bucket =
        new S3EventNotification.S3BucketEntity(
            INPUT_BUCKET_NAME, EMPTY_USER_IDENTITY, randomString());
    var object =
        new S3EventNotification.S3ObjectEntity(
            expectedObjectKey, SOME_FILE_SIZE, randomString(), randomString(), randomString());
