...
public class VerifyScopusEmailReceivedHandlerTest {
  ...
  void init() {
    ...
    handler = new VerifyScopusEmailReceivedHandler(s3Client, BUCKET_NAME);
  }
  ...
  void shouldEmitEventWhenThereIsNoObjectYoungerThan24hours() {
    ...
    assertThrows(
        NoScopusEmailsReceivedException.class,
  ...
  @SuppressWarnings("unchecked")
  private ResponseBytes<GetObjectResponse> returnObject(
      InvocationOnMock invocationOnMock, String content) throws Exception {
    var contentsAsByteArray = content.getBytes(StandardCharsets.UTF_8);
    var response =
        GetObjectResponse.builder().contentLength((long) contentsAsByteArray.length).build();
    var transformer =
        (ResponseTransformer<GetObjectResponse, ResponseBytes<GetObjectResponse>>)
            invocationOnMock.getArgument(1);
    return transformer.transform(
        response, AbortableInputStream.create(IoUtils.stringToStream(content)));
  }
  ...
  private List<S3Object> freshObject() {
    return List.of(S3Object.builder().key(OBJECT_KEY).lastModified(Instant.now()).build());
  }
}
