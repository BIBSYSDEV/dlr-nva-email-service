...
public class ZipFileRetriever implements FileRetriever {
  ...
  private InputStream sendRequest(URI uri) {
    return Try.of(
            () -> httpClient.send(createRequest(uri), HttpResponse.BodyHandlers.ofInputStream()))
        .mapTry(this::getBodyFromResponse)
        .get();
  }
  ...
  private InputStream getBodyFromResponse(HttpResponse<InputStream> response) {
    if (response.statusCode() != HttpURLConnection.HTTP_OK) {
      LOGGER.error(
          "Request failed with status code: {}, for url: {}",
          response.statusCode(),
  ...
  private HttpRequest createRequest(URI uri) {
    return HttpRequest.newBuilder().GET().setHeader("Accept", "application/zip").uri(uri).build();
  }
}
