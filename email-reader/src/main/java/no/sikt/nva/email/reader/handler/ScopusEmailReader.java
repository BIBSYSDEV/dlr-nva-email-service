...
public class ScopusEmailReader implements RequestHandler<S3Event, Set<URI>> {
  ...
  public static final String COULD_NOT_PERSIST_FILE_IN_S_3_BUCKET =
      "Could not persist file in s3 bucket";
  ...
  private Set<URI> downloadToBucketStorage(Set<URI> uris, S3Event event) {
    ...
    return uris.stream()
        .map(uri -> persistInBucket(uri, s3Driver, event))
  ...
  private URI persistFilesToS3(
      InputStream inputStream, UnixPath objectPath, S3Driver s3Driver, S3Event event) {
    try {
      return s3Driver.insertFile(objectPath, inputStream);
    } catch (Exception e) {
      LOGGER.error(COULD_NOT_PERSIST_FILE_IN_S_3_BUCKET, e);
      throw new EmailException(
          COULD_NOT_PERSIST_FILE_IN_S_3_BUCKET,
  ...
  private RuntimeException handleFailure(Throwable throwable, S3Event event) {
    return throwable instanceof EmailException emailException
        ? emailException
        : new EmailException(
            UNABLE_TO_DOWNLOAD_FILE, extractBucketName(event), extractObjectKey(event), throwable);
  }

  private Set<URI> extractUrisFromMessage(S3Event event, Message message) {
    var messageReader =
        new MultipartReader(message, extractBucketName(event), extractObjectKey(event));
  ...
  private void validateMessage(Message message, S3Event event) {
    var mimeValidator = new ScopusEmailValidator(extractBucketName(event), extractObjectKey(event));
  ...
  private URI createS3BucketUri(S3Event s3Event) {
    return URI.create(
        String.format("s3://%s/%s", extractBucketName(s3Event), extractObjectKey(s3Event)));
  }
