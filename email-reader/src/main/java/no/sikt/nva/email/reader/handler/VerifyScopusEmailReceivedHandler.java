...
public class VerifyScopusEmailReceivedHandler implements RequestHandler<ScheduledEvent, Void> {

  private static final Logger LOGGER =
      LoggerFactory.getLogger(VerifyScopusEmailReceivedHandler.class);
  ...
  public VerifyScopusEmailReceivedHandler() {
    this(S3Driver.defaultS3Client().build(), new Environment().readEnv("SCOPUS_EMAIL_BUCKET_NAME"));
  }

  public VerifyScopusEmailReceivedHandler(S3Client s3Client, String bucketName) {
  ...
  private boolean didNotReceiveScopusEmail() {
    return getObjectsYoungerThan24Hours().stream().noneMatch(this::validateScopusEmail);
  }
